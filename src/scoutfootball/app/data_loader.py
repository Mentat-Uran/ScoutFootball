...
class SyntheticDataError(RuntimeError):
    """Raised when synthetic fallback data reaches a path that requires real artifacts.

    PRS-0 R-003 (see ``docs/PLAYER_RATING_RESEARCH_SYSTEM_PLAN.md``) requires
    that synthetic fallback must not enter real research health, evaluation
    or export. ``assert_real_frame`` raises this exception when a frame
    stamped by ``_mark_synthetic`` is passed to a research/evaluation/export
    code path. The exception is caught at the API boundary and turned into a
    uniform error response so callers do not silently serve synthetic data
    as real research output.
    """


def frame_is_synthetic(df: pd.DataFrame | None) -> bool:
    """Return True if *df* was produced by the synthetic demo-data fallback.

    A frame is synthetic when it carries the ``is_synthetic`` column and at
    least one row is marked True. Demo data is generated by
    ``scoutfootball.app.demo_data`` and stamped via ``_mark_synthetic``; real
    artifacts never carry this column. Empty/None frames are not synthetic
    ...
    if "is_synthetic" not in df.columns:
        return False
    # Compare to True directly so NaN entries are treated as False without
    # triggering the pandas FutureWarning about downcasting object dtype
    # arrays on .fillna. Any row explicitly marked True means the frame is
    # synthetic.
    return bool(df["is_synthetic"].eq(True).any())


def assert_real_frame(df: pd.DataFrame | None, name: str) -> None:
    """Refuse synthetic fallback data in research/evaluation/export paths.

    PRS-0 R-003: synthetic fallback must not enter real research health,
    evaluation or export. Callers that require real artifacts should call
    this before serializing or persisting *df*. ``name`` is a short label
    (e.g. ``"player_ratings"``) included in the error message so the
    maintainer can identify which artifact is missing.
    """
    if frame_is_synthetic(df):
        raise SyntheticDataError(
            f"{name} contains synthetic fallback data; refusing to use it "
            f"for research/evaluation/export. The real artifact is missing "
            f"or unreadable. Run `scoutfootball build-features` and "
            f"`scoutfootball train` to produce real {name}."
        )
