...
| 球员评分 | 优化器、覆盖/可用性约束、holdout 指标、模型运行登记 | 部分交付 | v1.3.1/v1.3.2 后续完整重跑、独立真值与同切分晋级仍需核验 |
...
| 静态新鲜度 | `frontend/data_manifest.json` 记录的生成时间仍为 2026-06-23；`scripts/check_frontend_manifest.py` 检测到 3 个未记录新文件和 6 个大小漂移（stale 返回 1） | 构建时生成，超过阈值或契约不匹配即阻断（完整门禁留到 C1） |
