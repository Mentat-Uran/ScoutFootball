...
### G0-A：个人工作流与数据权利 — `ready`

- [x] 固化 [`PROJECT_CHARTER.md`](PROJECT_CHARTER.md)，并同步 README、路线图、能力表和开发规则。
- [ ] 分别记录球探决策、比赛准备、数据/模型研究的真实个人任务：输入、步骤、输出、现有替代工具、错误和阻断。
- [ ] 选择至少一个会重复使用的参考工作流，保存一次真实端到端运行和人工复盘证据；其余流程允许停止。
- [ ] 建立假设登记，记录证据、反证、置信度、下一测试和停止条件。
- [~] 建立实际输入的数据权利清单；未确认许可、保存、删除和导出边界的数据源不得进入后继节点。（[DATA_RIGHTS.md](DATA_RIGHTS.md) 初版已建，覆盖 13 个适配器；5 个数据源许可未确认，待维护者核对）

退出证据：章程与入口文档一致；至少一个参考工作流有真实任务证据；近期输入的权利和本地边界明确。
