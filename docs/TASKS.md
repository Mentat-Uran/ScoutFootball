...
> 当前队列更新：2026-07-17。项目定位见 [`PROJECT_CHARTER.md`](PROJECT_CHARTER.md)，长期依赖见 [`ROADMAP.md`](ROADMAP.md)，行业依据见 [`FOOTBALL_TOOLING_LANDSCAPE_2026.md`](FOOTBALL_TOOLING_LANDSCAPE_2026.md)，能力边界见 [`CAPABILITIES.md`](CAPABILITIES.md)。本文件顶部是当前任务真源；下方旧阶段和交付记录仅作历史证据，其中的日期不构成当前期限。
...
### G0-A：个人工作流与数据权利 — `in_progress`

- [x] 固化 [`PROJECT_CHARTER.md`](PROJECT_CHARTER.md)，并同步 README、路线图、能力表和开发规则。
- [ ] 分别记录球探决策、比赛准备、数据/模型研究的真实个人任务：输入、步骤、输出、现有替代工具、错误和阻断。（骨架见 [WORKFLOW_LOG.md](WORKFLOW_LOG.md)，待维护者填写真实任务）
- [ ] 选择至少一个会重复使用的参考工作流，保存一次真实端到端运行和人工复盘证据；其余流程允许停止。（骨架见 [WORKFLOW_LOG.md](WORKFLOW_LOG.md)，待维护者执行并记录）
...
退出证据：章程与入口文档一致（已满足）；至少一个参考工作流有真实任务证据（待维护者填写 [WORKFLOW_LOG.md](WORKFLOW_LOG.md)）；近期输入的权利和本地边界明确（已满足）。
...
- 为当前声明支持的参考工作流加入真实浏览器 E2E，覆盖 LIVE、STATIC、OFFLINE、空数据、低覆盖、字段缺失、移动阅读和导入安全。
- 构建时生成静态 manifest；关键 API/静态契约、文件新鲜度或序列化失败时阻断发布。

退出证据：参考工作流 fixture 通过；新决策包的外部事实和派生主张证据完整；API/静态同快照一致；入口文档口径一致。
...
- [x] **世界杯比赛智能分析（WC Match Intelligence — Player Spotlight + Team Form Trend + Cross-Stage Match Importance）**（2026-07-17，Round 104）：为 `/world-cup/tournament` 锦标赛视图新增 4 个互联的世界杯分析面板：单场球员聚光灯、球队状态趋势、跨阶段比赛重要性统一榜单、淘汰赛阶段比赛影响分析。后端 + 前端 + 测试改动，跨 `src/scoutfootball/api.py`、`src/scoutfootball/api_server.py`、`frontend/app.js`、`frontend/index.html` + 4 个新测试文件（共 39 个新测试）。(1) **后端 `get_wc_match_player_spotlight(home, away, top_n)` 函数**——`api.py` 新增函数，为指定世界杯对阵输出最可能影响比赛的 N 名球员。新增两个评分 helper：`_wc_spotlight_opponent_weakness(player_pos, opp_team)`（通过 `compute_position_gap_report` 汇总对手的位置组深度缺口）和 `_wc_spotlight_player_score(player, opponent, is_home, host_bonus)`（综合球员评分、位置权重 `_WC_SPOTLIGHT_POSITION_WEIGHTS`、俱乐部缺口置信度乘数 `_WC_SPOTLIGHT_CONFIDENCE_MULTIPLIER`、对手弱点，得到 0–100 聚光灯评分）。每球员含 `spotlight_score`/`position`/`club`/`rating`/`confidence`/`opponent_weakness`/`factors`。Defensive：`no_data` 当阵容数据不可用、`team_not_found` 当球队未知、`limitations` 数组明确描述性 overlay 边界。(2) **后端 `get_wc_team_form_trend(team, last_n)` 函数**——`api.py` 新增函数，使用 enriched `recent_form` 数据构建球队最近 N 场比赛的状态趋势。每场比赛含 `date`/`opponent`/`venue`/`kind`（home/away/neutral）/`outcome`（W/D/L）/`team_goals`/`opponent_goals`/`cumulative_points`，并附汇总统计（PPG、W/D/L 计数、净胜球）和 ECharts-ready xG 趋势序列。Defensive：未知球队或无近期比赛时返回 `no_data`，使用 `limitations` 数组（非 `disclaimer` 字符串）。(3) **后端 `get_wc_tournament_top_matches(group_top_n, knockout_top_n, num_simulations)` 函数**——`api.py` 新增函数，将 Round 103 小组赛阶段比赛影响与新增的淘汰赛阶段影响合并为统一榜单，按 `impact_metric` 排序。每条目含 `stage`（`group`/`knockout`）/`stage_label`/`match_id`/`home`/`away`/`date`/`venue`/`impact_metric`/`max_swing`/`max_swing_team`。Defensive：无未赛比赛时返回 `no_data`，使用 `limitations` 数组。(4) **后端 `get_wc_tournament_knockout_match_impact(num_simulations, top_n)` 函数**——`api.py` 新增函数，蒙特卡洛淘汰赛阶段影响分析器。对每个未赛 KO 比赛，使用模型派生的比分（通过 `_most_likely_wc_scoreline(prediction["score_matrix"])` 而非 Round 103 硬编码的 2-1/1-1/1-2）模拟三种确定性结果（主胜/平/客胜），然后运行实力加权锦标赛模拟计算每支球队的冠军概率摆幅。返回比赛按 `total_impact` 降序排序，每场含 `per_team` 数组（前 3 按摆幅排序）、`max_swing`、`max_swing_team`。(5) **修复 Round 103 的 `get_wc_tournament_match_impact` 回归 bug**——`m.get("home_team", "")` 改为 `m.get("home", "")`（与实际 `state.matches` 字段名匹配）；硬编码 2-1/1-1/1-2 比分替换为通过 `_most_likely_wc_scoreline` 从 Poisson 比分矩阵派生的模型比分。(6) **API 端点注册**——`api_server.py` 新增 4 个端点：`GET /world-cup/match-briefings/{home}/{away}/spotlight`（top_n 1–10）、`GET /world-cup/teams/{team}/form-trend`（last_n 1–20）、`GET /world-cup/tournament/top-matches`（group_top_n 1–20, knockout_top_n 1–20, num_simulations 100–10000）、`GET /world-cup/tournament/knockout/match-impact`（num_simulations 100–20000, top_n 1–50）。(7) **前端 state + fetch helpers**——`app.js` 扩展 `wcApiData` 含 `matchSpotlightCache`、`teamFormTrendCache`、`tournamentKnockoutMatchImpact`、`tournamentTopMatches`、`matchSpotlightLoading`、`teamFormTrendLoading`。4 个新 fetch helper 使用 `{ fetchOpts: { signal: AbortSignal.timeout(N) } }`（而非不支持的 `timeout` 选项）：`fetchWcMatchPlayerSpotlight`、`fetchWcTeamFormTrend`、`fetchWcTournamentKnockoutMatchImpact`、`fetchWcTournamentTopMatches`。`loadAndRenderWcTournament` 扩展为同时拉取 top matches。(8) **前端面板**——Top Matches 排行榜面板（含刷新按钮）和 KO Match Impact 面板（通过 `wc-ko-impact-load` 按钮懒加载）添加到 `renderWcTournament`；`_renderWcKoImpactBody(z)` helper 渲染 KO 影响表使用 `max_swing`/`max_swing_team`/`round`/`total_impact` 字段。新 `renderWcMatchSpotlightPanel(teamA, teamB)` 面板添加到 `renderWcCompare` 末尾，含懒加载按钮、加载状态、错误状态、球员表（含 `spotlight_score` 进度条）。新 `renderWcFormTrendPanel(team)` 面板添加到 `renderWcSchedule`，含球队下拉菜单、W/D/L form pills、统计摘要、ECharts xG 趋势图（通过 `setTimeout` 在 innerHTML 后初始化）、比赛表。(9) **HTML 容器**——`index.html` 在 `wc-match-prediction-panel` 后添加 `#wc-match-spotlight-panel`；在 WC 赛程表 article 后添加 Form Trend 面板 article（含 `#wc-form-trend-team` select、`#wc-form-trend-btn` 按钮、`#wc-form-trend-panel` div）。(10) **字段名校正**——所有新 API 使用 `limitations` 数组（非 `disclaimer` 字符串）；form trend 使用 `kind`（非 `recorded`）、`team_goals`/`opponent_goals`（非 `home_xg`/`away_xg`）；top matches 使用 `impact_metric`（非 `metric`）；KO match impact 不返回 `top_n` 字段（使用 `mode`/`status`）。前端同步这些字段名。**39 个新测试** 跨 4 个测试文件：`test_wc_match_player_spotlight.py`（3 类 9 测试）、`test_wc_team_form_trend.py`（4 类 12 测试）、`test_wc_top_matches.py`（2 类 10 测试）、`test_wc_knockout_match_impact.py`（2 类 8 测试）。既有测试仍通过：`test_worldcup_match_impact.py`（20 测试）、`test_worldcup_match_briefing.py`（28 测试）。`node --check frontend/app.js` clean；ruff clean。

- [x] **世界杯比赛影响分析（WC Match Impact Analysis）**（2026-07-17，Round 103）：为 `/world-cup/tournament` 锦标赛模拟器的小组积分榜新增"出线%"和"头名%"两列，由实力加权的蒙特卡洛模拟剩余小组赛驱动，用户无需切换面板即可看到每支球队的出线前景。后端 + 前端 + 测试改动。(1) **后端 `get_wc_tournament_standings_probabilities(group, num_simulations)` 函数**——`src/scoutfootball/api.py` 新增函数，复用既有 `simulate_group_stage(state, team_strengths, num_simulations, mode="strength", seed=42)` 进行实力加权蒙特卡洛模拟，构建 `prob_by_team` 查找映射，然后将 `advance_prob` 和 `win_group_prob` 注入到每支球队的积分榜行中。支持 group 过滤（返回单组）或全部 12 组；defensive：`unknown_group` 错误、阵容数据不可用时返回 `no_data` 状态、包含 `source_attribution` + `disclaimer`。返回：`{status, group, groups: {letter: enriched_rows}, num_simulations, remaining_matches, mode, source_attribution, disclaimer}`。(2) **API 端点注册**——`src/scoutfootball/api_server.py` 新增 `@app.get("/world-cup/tournament/standings-probabilities")` 端点，接受可选 `group` 和 `num_simulations`（默认 2000）查询参数，导入按字母序在 `get_wc_tournament_standings` 之后。(3) **前端 `fetchWcTournamentStandingsProbabilities(group)` 函数**——异步函数，缓存在 `wcApiData.tournamentStandingsProbabilities`，30s 超时，接受 `ok` 和 `no_data` 状态，错误时设为 null + console.warn，成功返回 data。(4) **积分榜集成**——`renderWcTournament` 构建 `probByTeam` 查找映射和 `hasProbData` 标志。数据可用时积分榜新增两列：`出线%`（Adv%，advance_prob）和 `头名%`（Win Grp%，win_group_prob），均为 `status-pill` 胶囊并按阈值分层着色（出线：>=0.7 高 / >=0.3 中 / 否则低；头名：>=0.5 高 / >=0.15 中 / 否则低）。数据不可用时不显示额外列（优雅降级）。表格下方显示免责声明 `wc_standings_prob_disclaimer`（数据可用时）或 `no_data` 免责声明（API 返回 no_data 时）。(5) **事件 handler 接线**——`fetchWcTournamentStandingsProbabilities` 在 5 处路径调用：`loadAndRenderWcTournament`（初始 Promise.all）、组选择 change handler、重置按钮 handler、录入结果按钮 handler、清除结果按钮 handler。所有路径通过 `renderWcTournament()` 重渲染。(6) **4 个新 i18n 键**（zh + en）：`wc_standings_prob_advance_col`、`wc_standings_prob_win_col`、`wc_standings_prob_disclaimer`、`wc_standings_prob_loading`。**16 个新后端测试** 在 `tests/unit/test_worldcup_standings_probabilities.py` 跨 4 个类（TestStandingsProbabilitiesStatus: 7 测试；TestStandingsProbabilitiesPerTeam: 6 测试；TestStandingsProbabilitiesDeterministic: 1 测试；TestStandingsProbabilitiesRemainingMatches: 2 测试）。**32 个新前端安全回归测试** 跨 4 个新测试类：`TestFetchWcTournamentStandingsProbabilities`（9 测试）、`TestWcStandingsProbabilitiesRender`（13 测试）、`TestWcStandingsProbabilitiesWiring`（6 测试）、`TestWcStandingsProbabilitiesI18n`（4 测试）。同时将 3 个 TestWcMatchPredictionWiring 正文切片上限从 12000 提升到 15000 以容纳 renderWcTournament 顶部新增的积分榜概率代码。`node --check frontend/app.js` clean；ruff clean；16 个后端测试通过；32 个前端安全测试通过（692 旧 + 32 新 = 724 总计）。
