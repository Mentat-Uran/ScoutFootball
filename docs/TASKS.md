...
- [x] **Riser/Decliner 交接 + 球员风格适配 + 球队簇招募推荐**（2026-07-14，Round 72）：3 个相互关联的球探工作流功能：(1) **Riser/Decliner → watchlist/shortlist 交接**——在 risers/decliners 表格每行新增操作列，包含 watchlist（□）和 shortlist（△）切换按钮。点击时构建带 `trajectory_signal` reason code（`rising_{slope}` 或 `declining_{slope}`）的球员对象，调用 `togglePlayerWatchlist()` / `togglePlayerShortlist()`（扩展为持久化 `reason_code`），然后重新渲染面板和球探视图。(2) **球员风格适配球队簇**——`compute_player_style_fit()` 在 `features/team_style.py` 计算球员 4 个风格组合特征（`npg_p90`/`assists_p90`/`defense_composite`/`possession_composite`）的分钟加权聚合，使用球队簇群体的 `feature_means` 和 `feature_stds` 标准化，然后对每个簇质心计算**余弦相似度**，映射到 0-100 适配分。`GET /players/{name}/style-fit` 端点。前端 scouting 视图新增面板：球员姓名输入 + 赛季/联赛过滤 + 逐簇适配分表（cluster_id/label/n_teams/fit_score/cosine_similarity/teams）+ 球员风格向量显示。(3) **球队簇 → 球员招募推荐**——`compute_cluster_recruits()` 对过滤后的 rating matrix 中每个球员计算与目标簇质心的余弦相似度适配分，按适配分降序排列，支持 position_group 过滤、min_player_minutes 阈值、exclude_cluster_teams 排除簇内球队。`GET /teams/style-clusters/recruits` 端点。前端 teams 视图簇表每行新增"招募"按钮，点击后展开招募结果表（排名/球员/球队/位置/适配分/评分/分钟/操作），每行可切换 watchlist/shortlist（`cluster_recruit_fit` reason code）。**25 个新单元测试**：10 个 style-fit（空输入/基本/球员未找到/大小写不敏感/排序/字段完整性/best_fit_cluster/disclaimer/insufficient_teams/无 mutation）+ 15 个 recruits（空输入/基本/排序/cluster_not_found/排除簇内球队/包含簇内球队/top_n cap/位置过滤/分钟过滤/n_scanned/disclaimer/无 mutation）。全部单元测试通过，ruff clean，node --check clean。前端安全测试通过（innerHTML 使用 `[...].join("")` 模式避免误报）。3 个新 i18n 键（zh + en）。
