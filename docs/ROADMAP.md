...
G0-A 与 G0-B 可以并行；G1 必须等待二者全部通过。三者均已验证（2026-07-17）；C1 已被解锁但未启动。当前状态以 [`TASKS.md`](TASKS.md) 顶部为准。
...
- 为 `frontend/app.js`、`api.py` 和 `api_server.py` 写模块边界 ADR；每次只拆一个能降低参考工作流风险的低耦合 seam，不承诺全面重构。
- 建立最小文档生成和陈旧度报告；更完整的 registry UI 留到 C1。
