...
def load_team_standings(settings):
    ...
    # Map season codes to our format
    season_map = {"2223": "2223", "2324": "2324", "2425": "2425"}
...
def params_to_weights(params):
    ...
    Layout:
      - position_weights: 8 positions × 5 dims = 40 params (softmax normalized)
      - attack_weights: 8 positions × 3 metrics = 24 params (softmax normalized)
      - availability sub-weights: 4 params (softmax)
      - quality sub-weights: 4 params (softmax)
      - league_log_scale: 1 param (scaling factor for league coefficient)
      - reliability_minutes_ref: 1 param (reference minutes for reliability)
      - reliability_starts_ref: 1 param (reference start rate for reliability)
    Total: 40 + 24 + 4 + 4 + 1 + 1 + 1 = 75
    """
    idx = 0
    ...
    # Reliability params
    rel_minutes_ref = 900 + params[idx] * 1800  # scale to [900, 2700]
    idx += 1
    rel_starts_ref = 0.3 + params[idx] * 0.4  # scale to [0.3, 0.7]
    idx += 1
...
def compute_ratings(player_df, weights):
    ...
    # Percentile computation per position
...
def objective(params, player_df, team_standings, verbose=False):
    """
...
def main():
    ...
    print(f"  基线 Spearman: {-baseline_loss:.4f}")

    # Optimize
    print("\n[3] 优化权重 (differential evolution, 可能需要几分钟)...")
    bounds = [(-3, 3)] * 75  # 75 parameters

    result = differential_evolution(
        objective,
        bounds=bounds,
        args=(player_df, team_standings),
        maxiter=50,
        popsize=15,
        tol=1e-4,
        seed=42,
    ...
    print(f"\n  优化完成!")
    print(f"  最终 Spearman: {-result.fun:.4f}")
    print(f"  提升: {-result.fun - (-baseline_loss):+.4f}")
...
def _get_default_params():
    ...
    # This is approximate — the exact inverse of softmax is not unique.
    # We use log-probabilities as a reasonable starting point.
    default_pw = {
