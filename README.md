...
The World Cup is a reference pack, not the permanent boundary of the core platform. The current first reference use is a reproducible, local, personal player-rating research system; recruitment, match preparation, action value, and tournament tools should reuse that evidence and research core rather than expand as independent product surfaces.

The focus now is to make player-rating research trustworthy: explicit rating targets, canonical identity and data grain, transparent baselines, independent evaluation labels, uncertainty, active-rating freshness, error analysis, and replayable local research packages. The detailed defects and dependency-gated plan are in [`docs/PLAYER_RATING_RESEARCH_SYSTEM_PLAN.md`](docs/PLAYER_RATING_RESEARCH_SYSTEM_PLAN.md).
...
- [`docs/PLAYER_RATING_RESEARCH_SYSTEM_PLAN.md`](docs/PLAYER_RATING_RESEARCH_SYSTEM_PLAN.md) defines the current player-rating research defects, target system, feature backlog, and PRS-0 through PRS-8 gates.
...
- **Player Ratings:** Research candidate based on a PyTorch team-points proxy optimizer, with holdout metrics, availability/quality caps, model-run registry, and local admission lifecycle. It is only partially delivered: the current active rating is older than the current feature matrix, no local run is currently reviewable, and there are no supervision-eligible independent labels. It must not be presented as validated player-ability truth.
