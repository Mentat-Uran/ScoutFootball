...
- **Truth Label Contracts:** Schema, validation, and a source-policy supervision audit for `player_truth_labels.parquet`. `expert_tier` labels derived from the current optimizer are excluded from NN training and optimizer anchors; source eligibility is still not proof of independent collection. Dated, locally supplied Transfermarkt CSV/Parquet snapshots can be previewed and imported with `scoutfootball import-transfermarkt-truth-labels`; no Transfermarkt scraping is implemented.
