可以融入，但要分层，不要全塞。ScoutLab 最适合吸收的是“事件数据标准化 + 行为价值模型 + 评分验证 + 可视化组件”，而不是再堆更多爬虫。

最优先推荐这几类。

1. socceraction：最值得接入

这是最应该融入 ScoutLab 的开源项目。它本身就是“足球动作价值评估工具包”，支持把 StatsBomb、Wyscout、Opta 等事件数据转成统一的 SPADL / atomic-SPADL 表示，并内置 xT、VAEP、Atomic-VAEP。它的定位和 ScoutLab 的“球员评分系统”高度重合。 ￼

你现在的 ScoutLab 评分主要还是赛季统计、FBref、Understat、Football-Data 这一类汇总数据。接入 socceraction 后，可以给球员评分增加“动作价值”维度：
...
这篇论文适合融入 ScoutLab 的“评分解释系统”。比如球员详情页不要只显示：
...
这会让 ScoutLab 从“统计打分器”升级成“可解释球探工具”。
...
建议不要只用它做少量事件样本，而是把它作为 ScoutLab 的“事件数据标准样板”。也就是说，所有 action-value、xT、VAEP、传球网络、shot map、pressure map 都先基于 StatsBomb Open Data 做出来。

可以新增：

src/scoutlab/adapters/statsbomb_event.py
src/scoutlab/features/action_features.py
src/scoutlab/visualization/pitch.py
...
ScoutLab 现在如果要提升“作品集观感”，mplsoccer 的性价比很高。建议做这些图：
...
这个方向很高级，但对 ScoutLab 不是短期任务。它适合以后作为“进攻过程质量”模块：
...
它对你最有价值的不是直接接入，而是参考它的数据抽象方式。ScoutLab 后面可以借鉴：
...
这一步会让 ScoutLab 从“赛季统计评分”升级为“动作价值评分”。
